            ...
            <div class="col-lg-4 col-xlg-3 col-md-5">
                <div class="card">
                    <div class="card-body">
                        <center class="mt-4">
                            @if (Auth::user()->image)
                                <img src="{{ asset('admin/upload/images/profile/' . Auth::user()->image) }}" class="rounded-circle"
                                    width="150" />
                            @else
                                <img src="{{ url('/') }}/admin/assets/images/users/profile.png" class="rounded-circle"
                                    width="150" />
                            @endif
...
@push('script')
    <script>
        $(document).ready(function() {
            $('#profile-form').on('submit', function(e) {
                ...
                $.ajax({
                    url: '{{ route('user.profile.update') }}', // Use the route name for URL
                    method: 'POST',
 ...
 if (!passwordRegex.test(newPassword)) {
    ...
    $.ajax({
        url: '{{ route("user.update-password") }}', // Adjust to your route
        method: 'POST',
        data: $(this).serialize(),
        success: function(response) {
            Swal.fire({
