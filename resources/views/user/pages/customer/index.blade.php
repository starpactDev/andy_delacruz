...
@section('content')
    ...
    <div class="row page-titles">
        <div class="col-md-5 col-12 align-self-center">
            <h3 class="text-themecolor mb-0">Managers List</h3>
                                ...
                                <tbody>
                                    @foreach ($managers as $manager)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <img src="{{ url('/') }}/admin/upload/images/managers/{{ $manager->user->image ?? 'profile.png' }}"
                        ...
                        <form id="createManagerForm">
                            <div class="modal-header d-flex align-items-center">
                                <h5 class="modal-title" id="createModalLabel">
                                    <i class="ti-marker-alt me-2"></i> Create New Manager
                                </h5>
    ...
    <script>
        $(document).ready(function() {
            $('#createmodel form').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('user.manager.store') }}", // Your route name for storing manager data
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        $(document).ready(function() {
            $('#editModal').on('show.bs.modal', function(event) {
                ...
                modal.find('#profilePreview').attr('src',
                    "{{ url('/') }}/admin/upload/images/managers/" + image);
            });
    ...
    $('.delete-row-btn').on('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route('user.manager.destroy', '') }}/' + userId, // Use route name
                    type: 'DELETE',
                    data: {
                    ...
                    success: function(response) {
                        Swal.fire(
                            'Deleted!',
                            'Manager Account has been deleted.',
                            'success'
                        );
                        location.reload(); // Reload the page to show updated data
                    },
                    error: function(xhr) {
                        Swal.fire(
                            'Error!',
                            'There was a problem deleting the Manager.',
                            'error'
                        );
                    }
                });
