        ...
        <div class="row">
            <div class="col-md-12">
                <div class="top-right-buttons">
                    @if ($orderDetails->label_count)
                    @php
                    $currentUser = auth()->user();
                    $userDriverInfo = $currentUser->driverInfo; // Assuming there's a relation defined between User and UserDriverInfo
                @endphp
                    @if (!($currentUser->type == 1 && optional($userDriverInfo)->team == 'Dominican Team'))


                        <button class="btn btn-warning" onclick="redirectToShipping('{{ $orderDetails->order_number }}')">
                    ...
                    <button class="btn btn-primary nav-item dropdown">
                        ...
                        <div class=" dropdown-menu dropdown-menu-end user-dd animated flipInY">
                            <a class="dropdown-item" id="gmailShareButton" href="#" target="_blank">
                                <i class="fas fa-envelope text-primary me-1 ms-1" style="font-size: 18px;"></i> 
                                Via Email
                            </a>

                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" data-bs-toggle="tooltip" id="whatsappShareButton" data-bs-placement="top">
                                <i class="fab fa-whatsapp feather-sm text-success me-1 ms-1" style="font-size: 18px;"></i>
                                Via Whatsapp
                            </a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" id="smsShareButton">
                                <i class="fas fa-comment-dots text-info me-1 ms-1" style="font-size: 18px;"></i>
                                Via SMS
                            </a>
                        </div>
                    </button>

                    <button class="btn btn-info" onclick="redirectToInvoices('{{ $orderDetails->order_number }}')">
                        <i class="fas fa-print"></i> Proceed to Print or Download
                    </button>

                    {{-- <button class="btn btn-secondary" onclick="redirectToEditInvoices('{{ $orderDetails->order_number }}')">
...
@push('script')
    <script>
        function redirectToInvoices(orderNumber) {
            window.location.href = '{{ route('driver.invoice.preview', ['order_number' => ':order_number']) }}'.replace(
    ...
    <script>
        function redirectToEditInvoices(orderNumber) {
            ...
            window.location.href = '{{ route('driver.invoice.edit', ':order_number') }}'.replace(
                ':order_number', orderNumber);
        }
    </script>
    <script>
        function redirectToShipping(orderNumber) {
            ...
            window.location.href = '{{ route('driver.barcode.generateAndRedirect', ':order_number') }}'.replace(
                ':order_number', orderNumber);
        }
    </script>
    ...
    <script>
        document.getElementById('gmailShareButton').addEventListener('click', function() {
            ...
            const invoiceLink = '{{ route('invoice.index', $orderDetails->order_number) }}';
    ...
    <script>
        document.getElementById('whatsappShareButton').addEventListener('click', function() {
            ...
            const message =
                `Dear Customer,\n\nPlease find the shipment details of your invoice at the following link:\n{{ route('invoice.index', $orderDetails->order_number) }}\n\nThank you !\nBest regards,\nEmbarque Paton Gomez`;
